...
export const GamePad = ({engine}: Props) => {
    ...
    return <div className="grid grid-rows-3 grid-cols-3 gap-1">
        ...
        {placingFences ?
            ...
            : <div />}
        <button onClick={commit} className="bg-blue-800 border border-gray-200 text-gray-200 rounded-md flex h-10 w-28 bg-theme-300">
            <p className="w-fit m-auto font-bold">
                { moveArmed ? 'COMMIT' :
        ...
        {placingFences ?
            <button onClick={left} className={`border border-gray-200 text-gray-200 rounded-md h-10 w-28 ${autoRotate ? 'invisible' : ''}`}>
                <svg className="m-auto h-[90%]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                    <path fillRule="evenodd" d="M7.72 12.53a.75.75 0 010-1.06l7.5-7.5a.75.75 0 111.06 1.06L9.31 12l6.97 6.97a.75.75 0 11-1.06 1.06l-7.5-7.5z" clipRule="evenodd" />
                </svg>
            </button>
            : <div />}
        <button onClick={placingFences ? rotate : () => { if (!autoRotate) next()}} className="border border-gray-200 text-gray-200 rounded-md h-10 w-28">
        ...
        {placingFences ?
            <button onClick={right} className={`border border-gray-200 text-gray-200 rounded-md h-10 w-28 ${autoRotate ? 'invisible' : ''}`}>
                <svg className="m-auto h-[90%]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
