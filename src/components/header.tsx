...
function HeaderButton( { link, text, color }: {link: string, text: string, color: string | undefined}) {
  return (
    <Link className={`text-center hover:text-theme-100 ${ color ? color : "text-gray-200"} text-xl font-bold py-4 px-12 hover:border-b-2 border-b-[1px] pb-[20px] hover:pb-[19px] font-sans`} href={link}>
    {text}
    </Link>
  )
}
